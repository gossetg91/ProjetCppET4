...
bool Unit::checkMove() {
	...
	if (relatedTeam->isRight() && ptile->getPrec() != nullptr) {
		if (ptile->getPrec()->isAnyBase()) return false; //pas le droit d'�tre dans la case de la base
		if (ptile->getPrec()->isEmpty()) return true;
	}
	else if (ptile->getNext() != nullptr) {
		if (ptile->getNext()->isAnyBase()) return false; //pas le droit d'�tre dans la case de la base
...
Tile* Unit::checkAttack() {
	...
	for (int i = minRange; i <= maxRange; i++) {
		...
		if (curElem != nullptr)
		{
			...
			else if (curElem->isAnyBase()) { //si il y a une base (ne sera jamais de m�me team)
				return curElem;
			}

			//on incr�mente la case vis�e
